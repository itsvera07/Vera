...
export default async function TopicPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [topicResult, { user }] = await Promise.all([payload.find({ collection: "topics", where: { slug: { equals: slug } }, limit: 1 }), payload.auth({ headers: await getHeaders() })]);
  ...
  const lessonCounts = await Promise.all(modulesResult.docs.map((m: any) => payload.find({ collection: "lessons", where: { module: { equals: m.id } }, limit: 100, sort: "orderInModule" })));
      ...
      <PageContainer>
        ...
        <section className="bg-white rounded-card p-4 shadow-card mt-4 animate-fade-in-up" style={{ animationDelay: "80ms" }}>
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className="font-semibold">{totalLessons ? Math.round((completedCount / totalLessons) * 100) : 0}%</span>
          </div>
          <ProgressBar percent={totalLessons ? (completedCount / totalLessons) * 100 : 0} fillClassName={theme.progressFill} />
        ...
        <section className="mt-6">
          ...
          <div className="flex flex-col gap-3 lg:grid lg:grid-cols-2 lg:gap-4">
            {modulesResult.docs.map((mod: any, i: number) => {
              ...
              const isLocked = mod.unlockRule === "sequential" && previousModule && previousDone < previousLessons.length;

              const isComplete = lessons.length > 0 && doneInModule === lessons.length;

              return (
                <Link key={mod.id} href={isLocked ? "#" : `/topics/${topic.slug}/${mod.slug}`} style={{ animationDelay: `${120 + i * 60}ms` }} className={`bg-white rounded-card p-4 shadow-card flex items-start gap-3 animate-fade-in-up transition-all duration-300 ease-smooth ${isLocked ? "opacity-60 pointer-events-none" : "hover:-translate-y-1 hover:shadow-lift active:scale-[0.99]"}`}>
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 transition-colors duration-300 ${isComplete ? "bg-navy text-white" : `${theme.progressFill} text-white`}`}>{isComplete ? <Check size={15} /> : i + 1}</span>
                  <div className="flex-1">
                    ...
                    <ProgressBar percent={lessons.length ? (doneInModule / lessons.length) * 100 : 0} fillClassName={theme.progressFill} />
                  </div>
                </Link>
              );
            })}

            {modulesResult.docs.length === 0 && <p className="text-sm text-ink-muted py-6 text-center lg:col-span-2">No modules yet for this topic — add some in /admin.</p>}
          </div>
        </section>
      </PageContainer>
    </>
