...
export default async function LessonPage({
  ...
  const [topicResult, moduleResult, lessonResult, { user }] = await Promise.all([
    ...
    payload.find({ collection: 'lessons', where: { slug: { equals: lessonSlug } }, limit: 1, depth: 2 }),
  ...
  const relatedChats = hasAccess
    ? await payload.find({ collection: 'chats', where: { linkedLesson: { equals: lesson.id } }, limit: 1 })
    : null
    ...
    <>
      <BreadcrumbHeader
        crumbs={[
          ...
          { label: 'Module', href: `/topics/${topic.slug}/${mod.slug}` },
          { label: `Lesson ${lesson.orderInModule}` },
        ]}
        backHref={`/topics/${topic.slug}/${mod.slug}`}
      />

      <PageContainer>
        <ProgressBar
          percent={((currentIndex + 1) / siblingLessons.totalDocs) * 100}
        ...
        <div className="mt-5 lg:max-w-2xl">
          {hasAccess ? (
            <LessonBlockRenderer blocks={lesson.content ?? []} accentClass={theme.iconText} linkedChatId={linkedChatId} />
          ) : (
            <TopicPaywall
              topicId={topic.id}
              topicTitle={topic.title}
              price={topic.unlockPrice}
        ...
        {hasAccess && (
          <div className="mt-6 lg:max-w-2xl">
            <CompleteLessonButton
              lessonId={lesson.id}
              nextHref={nextLesson ? `/topics/${topic.slug}/${mod.slug}/${nextLesson.slug}` : undefined}
              accentClass={theme.progressFill}
            />
            <div className="flex gap-3 mt-3">
              {previousLesson && (
                <a
                  href={`/topics/${topic.slug}/${mod.slug}/${previousLesson.slug}`}
                  className="flex-1 flex items-center justify-center gap-1.5 text-center border border-ink/20 rounded-pill py-2.5 text-sm font-medium transition-all duration-200 ease-smooth hover:border-ink hover:bg-white"
                >
                  <ArrowLeft size={14} /> Lesson {previousLesson.orderInModule}
                </a>
              )}
              <a
                href={`/topics/${topic.slug}/${mod.slug}`}
                className="flex-1 text-center border border-ink/20 rounded-pill py-2.5 text-sm font-medium transition-all duration-200 ease-smooth hover:border-ink hover:bg-white"
              >
                Back to Module
              </a>
            </div>
