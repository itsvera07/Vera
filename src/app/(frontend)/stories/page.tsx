...
export default async function StoriesPage() {
  ...
  const booksByTheme = await Promise.all(
    themes.docs.map((t: any) => payload.find({ collection: 'books', where: { theme: { equals: t.id } }, limit: 20 })),
  )
    ...
    <>
      ...
      <PageContainer>
              ...
              {booksByTheme[i].docs.map((book: any, j: number) => (
                <ScrollReveal key={book.id} delay={j * 0.05} className="min-w-[140px] lg:min-w-0 shrink-0">
                  <TiltLink
                    ...
                    className="bg-white rounded-card shadow-card p-3 transition-shadow duration-300 hover:shadow-lift"
                  >
                    ...
                    <p className="text-xs text-ink-muted mt-1">
                      {book.releaseSchedule === 'weekly' ? 'New chapter weekly' : 'All chapters available'}
                    </p>
                  </TiltLink>
                </ScrollReveal>
              ))}
              {booksByTheme[i].docs.length === 0 && (
                <p className="text-sm text-ink-muted">No books yet in this theme.</p>
              )}
            </div>
          </section>
        ))}
        ...
        {themes.docs.length === 0 && (
          <p className="text-sm text-ink-muted py-6 text-center">No story themes yet — add some in /admin.</p>
        )}
      </PageContainer>
    </>
  )
