...
export default async function ChatLibraryPage() {
  ...
  const chatsByTheme = await Promise.all(themes.docs.map((t: any) => payload.find({ collection: "chats", where: { theme: { equals: t.id } }, limit: 20 })));
          ...
          <section key={theme.id} id={`theme-${theme.slug}`} className="mb-8 scroll-mt-6">
            ...
            <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 lg:gap-4">
              {chatsByTheme[i].docs.map((chat: any) => (
                <div key={chat.id} id={`chat-${chat.id}`} className="scroll-mt-20 bg-white rounded-card p-4 shadow-card transition-all duration-300 ease-smooth hover:shadow-hover hover:-translate-y-0.5 target:ring-2 target:ring-brand-orange">
              ...
              {chatsByTheme[i].docs.length === 0 && <p className="text-sm text-ink-muted">No chats added yet for this theme.</p>}
            </div>
          </section>
        ))}

        {themes.docs.length === 0 && <p className="text-sm text-ink-muted py-6 text-center">No chat themes yet — add some in /admin.</p>}
      </PageContainer>
    </>
  );
}
