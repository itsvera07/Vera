      ...
      <PageContainer>
        <div className="bg-navy rounded-card p-5 text-white flex items-center justify-between animate-fade-in-up">
          ...
          <Link
            ...
            className="bg-brand-orange text-white text-sm font-semibold rounded-pill px-4 py-2.5 flex items-center gap-1.5 transition-all duration-200 ease-smooth hover:-translate-y-0.5"
          >
            <Wallet size={14} /> Top up
          </Link>
        </div>

        <p className="text-xs text-ink-muted mt-3 lg:max-w-md">
          Vera uses pay-as-you-go wallet credits rather than a recurring
          subscription — you only ever pay for what you unlock.
        </p>
            ...
            <div key={p.id} className="bg-white rounded-card p-4 shadow-card flex items-center justify-between">
              <div>
                <p className="text-sm font-medium">
                  {p.itemType === 'topic' ? p.topic?.title ?? 'Topic' : p.book?.title ?? 'Book'}
                </p>
