...
export function HomeHeader() {
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-cream/70 flex items-center justify-between px-5 lg:px-0 lg:max-w-3xl xl:max-w-4xl lg:mx-auto pt-5 pb-2">
      ...
      <Link href="/search" aria-label="Search" className="w-9 h-9 flex items-center justify-center rounded-full transition-all duration-200 hover:bg-black/5 hover:scale-105">
        <Search size={19} />
      </Link>
    </header>
  );
...
export function BreadcrumbHeader({ crumbs, backHref }: { crumbs: { label: string; href?: string }[]; backHref: string }) {
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-cream/70 flex items-center justify-between px-5 lg:px-0 lg:max-w-3xl xl:max-w-4xl lg:mx-auto pt-5 pb-3">
      <Link href={backHref} aria-label="Back" className="w-9 h-9 rounded-full bg-white shadow-card flex items-center justify-center transition-all duration-200 ease-smooth hover:shadow-hover hover:-translate-x-0.5">
        <ArrowLeft size={17} />
      </Link>
