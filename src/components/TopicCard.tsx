...
export function TopicCard({
  ...
  return (
    <ScrollReveal delay={index * 0.05} className={featured ? 'lg:row-span-2' : ''}>
      <TiltLink
        href={href}
        className={`group rounded-card p-4 ${featured ? 'lg:p-7 lg:h-full lg:flex lg:flex-col lg:justify-end' : ''} transition-shadow duration-300 hover:shadow-lift ${
          CARD_COLOR_CLASSES[cardColor] ?? 'bg-pastel-peach'
        }`}
      >
        <span
          className={`inline-flex w-10 h-10 ${
            featured ? 'lg:w-16 lg:h-16' : ''
          } rounded-xl bg-white/70 items-center justify-center mb-6 transition-transform duration-300 ease-smooth group-hover:scale-110 group-hover:rotate-3`}
        >
