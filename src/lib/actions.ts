...
export async function unlockTopic(topicId: string | number) {
  ...
  const alreadyUnlocked = (user.unlockedTopics ?? []).some((t: any) => (typeof t === "string" ? t : t.id) === topicId);
...
export async function unlockBook(bookId: string | number) {
  ...
  const alreadyUnlocked = (user.unlockedBooks ?? []).some((b: any) => (typeof b === "string" ? b : b.id) === bookId);
  if (alreadyUnlocked) return { ok: true };
...
export async function markLessonComplete(lessonId: string | number) {
  ...
  const lessonIdNum = typeof lessonId === "string" ? Number(lessonId) : lessonId;

  const progress = [...(user.lessonProgress ?? [])];
  const existingIndex = progress.findIndex((p: any) => String(typeof p.lesson === "object" ? p.lesson?.id : p.lesson) === String(lessonIdNum));

  if (existingIndex >= 0) {
    progress[existingIndex] = { ...progress[existingIndex], completed: true, lastOpenedAt: new Date().toISOString() };
  } else {
    progress.push({ lesson: lessonIdNum, completed: true, lastOpenedAt: new Date().toISOString() });
  }
